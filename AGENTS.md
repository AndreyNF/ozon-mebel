...
5. **Серия фото (обязательно)** — **10 кадров 3:4** в `cards/{АРТ}/images/` (`01-main` … `10-*`). **i2i от фото производителя** — не выдумывать мебель: можно фон, lifestyle, людей; **нельзя** лишние шкафы/модули и неверный цвет. Правило: `.cursor/rules/ozon-photo-fidelity.mdc`, промпты `docs/prompts-by-category.md`. **Кухня:** слоты `08-cooktop-gas`, `09-cooktop-electric`, `10-cooktop-induction`. Без 10 фото **не загружать** в Ozon.
