...
def credentials() -> tuple[str, str]:
    ...
    if not client_id or not api_key:
        raise SystemExit(
            "Задайте OZON_CLIENT_ID и OZON_API_KEY в .env (см. .env.example, docs/ozon-api-setup.md)"
        )
    return client_id, api_key


def post(path: str, body: dict[str, Any] | None = None, *, timeout: float = 60.0) -> dict[str, Any]:
